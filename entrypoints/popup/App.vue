<template>
  <div flex="~ col" font-sans bg="grey-50" class="app-container">
    ...
    <div
      v-if="tasks.has_connect_error"
      flex="~ col items-center justify-center"
      flex-1
      p="4"
      text="red-500">
      <div i-mdi-desktop-classic text="4xl" mb-3 />
      <p text="lg" font="normal">{{ t('failedToConnectToNativePort') }}</p>
      <p text="sm" font="normal" mt-2 text-center>{{ t('checkNativeMessagingConfig') }}</p>
    </div>
    <div v-else>
      <main>
        <section p="x-4 y-3">
          ...
          <div v-if="activeTasks.length > 0">
            <ActiveItem
              ...
              :key="task.id"
              :task="task"
              @resume="tasks.resumeTask"
              @pause="tasks.pauseTask"
              @cancel="tasks.cancelTask" />
          </div>
          <div v-else class="empty-placeholder" text-center p="y-4" text="sm gray-500">{{ t('noDownloadTasks') }}</div>
        </section>
      </main>
      <div v-if="deactiveTasks.length > 0">
        <hr border="t gray-200" my="2" />
        <section p="x-4 y-3">
          <h2 font="semibold" text="md gray-700" mb="3">{{ t('completed') }}</h2>
          <div>
            <DeactiveItem
              v-for="task in deactiveTasks"
              :key="task.id"
              :task="task"
              @open-file="tasks.openFile"
              @open-folder="tasks.openFolder"
              @remove-record="tasks.removeRecord"
              @retry="tasks.resumeTask"
...
const { active: activeTasks, deactive: deactiveTasks } = storeToRefs(tasks)

onMounted(() => {
  tasks.listenForUpdates()
})
</script>
