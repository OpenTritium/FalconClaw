...
const statusInfo = computed(() =>
  match(props.task)
    .with({ status: TaskStatus.COMPLETED, size: P.select(P.number.gt(0)) }, size => prettyBytes(size))
    .with({ status: TaskStatus.COMPLETED }, () => t('finished'))
    .with({ status: TaskStatus.FAILED }, task => task.last_error)
    .with({ status: TaskStatus.CANCELLED }, () => t('taskCancelled'))
    .otherwise(() => null)
)
</script>
