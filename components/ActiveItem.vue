<template>
<div h="12" relative mb="2" rounded="lg" border="~ gray-200" bg-white transition="all duration-200 ease"
  hover="translate-y--1px shadow-md">
  <div absolute inset-0 rounded-lg overflow-hidden z="0" bg="gray-100">
    <div class="progress-fill" absolute top-0 left-0 h-full transition="all 0.3s ease" :class="progressBarColor"
  ...
  <div relative flex="~ items-center" h-full p="x-3" z="10">
    ...
    <div flex="~ gap-2">
      <button v-if="task.status === TaskStatus.PAUSED" @click="emit('resume', task.id)" i-mdi-play-circle-outline
        btn-action title="恢复" />
      <button v-else @click="emit('pause', task.id)" i-mdi-pause-circle-outline btn-action title="暂停" />
      <button @click="emit('cancel', task.id)" i-mdi-close-circle-outline btn-action title="取消" />
    </div>
  </div>
</div>
</template>
...
import { type TaskInfo, type TaskId, TaskStatus, progress_in_percentage } from '@/types'
